 ...
 * @brief Lightweight test harness for L-BFGS using cuBLAS and CUDA kernels.
 * @version 1.0
 * @date 2026-02-10
 * 
...
/**
 * @brief Kernel that updates the solution vector: x[i] += alpha * d[i]
 *
 * @param n Number of elements in the vectors.
 * @param x Device pointer to the solution vector to be updated (in-place).
 * @param d Device pointer to the search direction vector.
 * @param alpha Scalar step length to apply to the search direction.
 */
...
/**
 * @brief Kernel to compute L-BFGS history vectors s = x_new - x_old and y = g_new - g_old.
 *
 * @param n Number of elements.
 * @param s Device pointer where s = x_new - x_old will be written.
 * @param y Device pointer where y = g_new - g_old will be written.
 * @param x_new Device pointer to new solution vector.
 * @param x_old Device pointer to previous solution vector.
 * @param g_new Device pointer to new gradient vector.
 * @param g_old Device pointer to previous gradient vector.
 */
...
/**
 * @brief Kernel to scale a vector: r = H0 * q
 *
 * @param n Number of elements.
 * @param r Output device vector where the scaled values are stored.
 * @param q Input device vector to be scaled.
 * @param H0 Scaling factor (often an estimate of initial Hessian diagonal).
 */
...
/**
 * @brief Simple kernel to fill an array with ones (used for dot-product via cuBLAS).
 *
 * @param a Device array to be filled with ones.
 * @param n Number of elements.
 */
...
/**
 * @brief Compute the sum of a device float vector using cuBLAS dot product with a vector of ones.
 *
 * This helper maintains a device buffer of ones (`d_ones`) to compute the true sum
 * as s = dot(d_v, ones), reusing the buffer across calls to avoid repeated allocations.
 *
 * @param h cuBLAS handle used to perform the dot product.
 * @param d_v Device pointer to the input vector.
 * @param n Number of elements in the vector.
 * @return float The computed sum (single precision).
 */
...
/**
 * @brief A minimal L-BFGS optimizer that uses cuBLAS for linear algebra and
 * custom CUDA kernels for small elementwise operations.
 *
 * The template parameter `Func` must provide `f`, `df`, and `set_handle`
 * methods so the optimizer can evaluate the objective and its gradient on
 * device memory and configure its cuBLAS handle.
 */
...
class lbfgs {
    ...
    /**
     * @brief Allocate device resources required by the optimizer.
     *
     * @param problem_size Dimension of the optimization problem.
     * @param memory_size L-BFGS history size (m).
     */
    ...
    /**
     * @brief Release device resources and destroy the cuBLAS handle.
     */
    void destroy() {
        cudaFree(d_x);
        cudaFree(d_x_old);
...
public:
   /**
    * @brief Run the L-BFGS optimization.
    *
    * @param problem_size Dimension of the problem (n).
    * @param memory_size History size (m).
    * @param x0 Host pointer to initial guess; result is written back on return.
    * @param max_itr Maximum number of iterations.
    * @param func Objective wrapper providing `f`, `df`, and `set_handle`.
    * @param eps Convergence tolerance for gradient norm.
    * @return double Final function value.
    */
...
/**
 * @brief Kernel computing quadratic function f(x)=sum(x_i^2) and gradient g_i = 2*x_i.
 *
 * @tparam T Floating point type used for computations.
 * @param x Input vector on the device.
 * @param f_vals Output array for per-element function contributions (optional).
 * @param g Output gradient array (optional).
 * @param n Number of elements.
 */
template <typename T>
 ...
 *
 * Allocates temporary GPU memory for per-element function contributions and
 * provides `f` and `df` methods compatible with the optimizer interface.
 *
...
/**
 * @brief Kernel computing Rosenbrock function contributions and gradient.
 *
 * f_i = 100*(x_{i+1} - x_i^2)^2 + (1 - x_i)^2 and gradient contributions are
 * accumulated with atomicAdd to handle overlapping indices.
 *
 * @tparam T Floating point type used for computations.
 * @param x Input vector on device.
 * @param f_vals Output per-element function contributions (optional).
 * @param g Output gradient array (optional; uses atomics if provided).
 * @param n Number of elements.
 */
template <typename T>
...
/**
 * @brief Wrapper for the Rosenbrock test function.
 *
 * Provides `f` and `df` methods that execute device kernels and reduce
 * per-element contributions using cuBLAS.
 *
 * @tparam T Floating point type used on device.
 */
template <typename T>
...
/**
 * @brief Non-atomic Rosenbrock kernel variant that computes function and gradient
 * without atomics. Useful for testing alternative gradient accumulation strategies.
 *
 ...
 *
 * Mirrors `RosenbrockTest` but uses the non-atomic kernel variant.
 *
 * @tparam T Floating point type used on device.
 */
template <typename T>
 ...
 *
 * f(x) = sum(x_i^2 - 10*cos(2*pi*x_i)) + 10*n
 *
 * @tparam T Floating point type.
 * @param x Input device vector.
 * @param f_vals Output per-element function contributions (optional).
 ...
 *
 * Executes the device kernel and reduces per-element values via cuBLAS.
 *
 * @tparam T Floating point type.
 */
template <typename T>
 ...
 *
 * Uses standard Ackley formulation; per-element contributions are written to
 * `f_vals` and gradient contributions to `g` when those pointers are provided.
 *
 * @tparam T Floating point type.
 * @param x Input device vector.
 * @param f_vals Output per-element function contributions (optional).
 ...
 *
 * Allocates temporary storage and provides `f`/`df` methods compatible with
...
/**
 * @brief Simple host-based quadratic objective for quick testing.
 *
 * Provides `f` and `df` methods that operate on device pointers. This
 * implementation copies data back to host for evaluation (cheap but not
...
/**
 * @brief Entry point: runs several optimization tests (Quadratic, Rosenbrock,
 * Rastrigin, Ackley) using the L-BFGS harness.
 *
 * Initializes test problems, runs the optimizer for each, and prints timing
 * and final objective values to stdout.
 */
int main() {
    using T = float;
